...
def entry(argv=None):
    ...
    parser.add_argument(
        ...
        dest="dataset_name",
        default="",
        help=("Name of the dataset. " "dataset_name.npz must be in dataset_path. "),
    )

    parser.add_argument(
        "--output", dest="output", default="/tmp/graph_configs.json", help="Location to write output files."
    )

    parser.add_argument("--gcp_pname", dest="gcp_pname", default="research-graph")

    parser.add_argument("--gcs_auth", dest="gcs_auth", default=None)
    ...
    parser.add_argument(
        "--gin_config", dest="gin_config", default="", help="Location of gin config (/app/configs = /src/configs)."
    )
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        dataframe_rows = (
            p
            | "Enumerate hyperparameter gridpoints." >> beam.Create(enumerate_configs())
            | "Test GCN." >> beam.ParDo(hparam_handler.GetGcnTester())
            | "Write JSON" >> beam.io.WriteToText(os.path.join(args.output, "results.ndjson"), num_shards=10)
        )
