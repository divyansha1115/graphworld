...
class GcnTester(beam.DoFn):
    ...
    def process(self, test_config):
        ...
        if self._sim:
            data, _ = get_sbm_from_torchgeo_data(data, os.path.join(self._dataset_path, self._sim_adj_file))
        ...
        if test_config["index"] == 0:
            ...
            output.update(
                NodeLabelMetrics(dataset.gt_data["gt_graph"], dataset.gt_data["labels"], dataset.gt_data["features"])
            )
        yield output
