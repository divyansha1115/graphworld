...
class NpzDataset(InMemoryDataset):
    ...
    def _get_gt_data(self):
        with open(osp.join(self.raw_dir, self.raw_file_names[0]), "rb") as infile:
            adj_matrix, attr_matrix, labels, label_mask = _load_npz_to_sparse_graph(infile)
        self.gt_data = {"gt_graph": _get_gt_graph(adj_matrix), "labels": labels, "features": attr_matrix}
